    ...
    <div class="mb-20">
        ...
        <form method="post" action="{{ route('test.answer.submit') }}">
            ...
            <div class="inner">
                ...
                <textarea disabled="disabled" value="@if(isset($testSubmitted)) {{ getTestAnswer($testSubmitted->id,$question->id)->question_answer ?? '' }} @endif" class="form-control" rows="3" cols="30" name="answer[{{ $question->id }}]" placeholder="Answer"></textarea>
            </div>
            </div>
            @php
            $i++;
