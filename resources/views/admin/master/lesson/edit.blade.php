                        ...
                        <form action="{{ route('lessons.update', $lesson->id) }}" method="post" enctype="multipart/form-data">
                            ...
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control" name="description">{{ old('description', $lesson->description) }}</textarea>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="status" class="form-control">
                                        ...
                                        @foreach (getActiveStatus() as $key => $item)
                                            <option value="{{ $key }}" @if(isset($lesson->status)) @if($lesson->status==$key) selected @endif @elseif(old('status')==$key) selected @elseif($key==1) selected @endif>{{ $item }}</option>
                                        @endforeach
                                        @endif
                                    </select>
