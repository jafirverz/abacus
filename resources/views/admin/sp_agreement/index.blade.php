...
<div class="main-content">
    <section class="section">
        <div class="section-header">
            <h1>{{ $title ?? '-' }}</h1>
            @include('admin.inc.breadcrumb', ['breadcrumbs' => Breadcrumbs::generate('admin_sp_agreement')])
        </div>
        <br />
        ...
        <div class="section-body">
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            <form action="{{ route('sp-agreement.search') }}" method="get">
                                @csrf
                                <div class="row">
                                    <div class="col-lg-4">
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            ...
                            <a href="{{ route('sp-agreement-archive.index') }}" class="btn btn-warning ml-2">Archived List</a>
                            @if(empty(Session::get('docusign_auth_code')))
                            <a href="{{route('connect.docusign')}}" class="btn btn-warning ml-2">Connect Docusign</a>
                            {{-- @else 
                                ...
                                <table class="table table-md">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{ route('sp-agreement.show', $item->id) }}"
                                                    class="btn btn-info mr-1 mt-1" data-toggle="tooltip"
                                                    data-original-title="View"><i class="fas fa-eye"></i></a>
                                                <a href="{{ route('sp-agreement.edit', $item->id) }}"
                                                    class="btn btn-light mr-1 mt-1" data-toggle="tooltip"
                                                    data-original-title="Edit"><i class="fas fa-edit"></i></a>
                                                @php
                                                $buyer_particular = App\BuyerParticular::where('seller_particular_id', $item->id)->first();
                                                @endphp
                                                @if(isset($buyer_particular->pdf_name))
                                                <a href="{{url('pdf/'.$buyer_particular->pdf_name)}}"
                                                        class="btn btn-light mr-1 mt-1" data-toggle="tooltip"
                                                        data-original-title="Edit"><i class="fas fa-file-download mr-1"></i></a>
                                                @endif
                                            </td>
                                            ...
                                            <td>{{ optional($item->vehicleparticular)->make ?? '' . ' ' . optional($item->vehicleparticular)->model ?? ''}}</td>
