    ...
    <section class="section">
        <div class="section-header">
            <div class="section-header-back">
                ...
                <a href="{{ url('upload-file/CompetitionPassUpload.xlsx') }}" class="btn btn-primary"><i class="fas fa-file-excel"></i> Download Sample</a>
            </div>
            <h1>{{ $title ?? '-' }}</h1>
        ...
        <div class="section-body">
          @include('admin.inc.messages')
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <form action="{{ route('comp.pass.store') }}" method="post" enctype="multipart/form-data">
                            @csrf
                            @method('POST')
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="competitionn" class="form-control" id="competition" onchange="this.options[this.selectedIndex].value && (window.location = this.options[this.selectedIndex].value);">
                                        ...
                                        <option value="<?php echo url('/'); ?>/admin/comp-pass-upload?comp_id={{ $comp->id }}&type={{ $comp->competition_type }}" data-comp="{{ $comp->competition_type }}" @if(isset($_GET['comp_id']) && $_GET['comp_id']==$comp->id) selected @endif>{{ $comp->title }}</option>
                                        @endforeach
                                    </select>
