    ...
    <section class="section">
        <div class="section-header">
            ...
            <div class="section-header-button">
                <a href="{{ route('chat-window.create') }}" class="btn btn-primary">Add New</a>
            </div>
            @include('admin.inc.breadcrumb', ['breadcrumbs' => Breadcrumbs::generate('admin_chat_window')])
        </div>
                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            <div class="card-header-form form-inline">
                                <form action="{{ route('chat-window.search') }}" method="get">
                                    @csrf
                                ...
                                <table class="table table-md">
                                    ...
                                    <tbody>
                                        @if($chatWindow->count())
                                            ...
                                            {
                                                $i=$chatWindow->perPage() * ($chatWindow->currentPage() - 1);
                                            }
                                            else
                                            {
                                                 $i=0;
                                        ...
                                        @foreach ($chatWindow as $key => $item)
                                        @php  $i++;@endphp
                                        <tr>
                                            <td scope="row">
                                            ...
                                            <td>
                                                <a href="{{ route('chat-window.show', $item->id) }}" class="btn btn-info mr-1 mt-1" data-toggle="tooltip" data-original-title="View"><i class="fas fa-eye"></i></a>
                                                <a href="{{ route('chat-window.edit', $item->id) }}" class="btn btn-light mr-1 mt-1" data-toggle="tooltip" data-original-title="Edit"><i class="fas fa-edit"></i></a>
                                            </td>
                                            <td>{{ $i }}</td>
                                            <td>{{ $item->title }}</td>
                                            <td><img src="{{ asset($item->icon) }}" alt="" width="200px"></td>
                        ...
                        @if(request()->get('_token'))
                            {{ $chatWindow->appends(['_token' => request()->get('_token'),'search' => request()->get('search') ])->links() }}
                        @else
                            {{ $chatWindow->links() }}
                        @endif
                        </div>
