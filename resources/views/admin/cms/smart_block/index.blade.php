    ...
    <section class="section">
        <div class="section-header">
            ...
            {{-- <div class="section-header-button">
                <a href="{{ route('smart-block.create') }}" class="btn btn-primary">Add New</a>
            </div> --}}
            @include('admin.inc.breadcrumb', ['breadcrumbs' => Breadcrumbs::generate('admin_smart_block')])
        </div>
                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            <div class="card-header-form form-inline">
                                <form action="{{ route('smart-block.search') }}" method="get">
                                    @csrf
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-md">
                                    ...
                                    <tbody>
                                        @if($smart_block->count())
                                        @foreach ($smart_block as $key => $item)
                                        <tr>
                                           {{--  <td scope="row">
                                            ...
                                            <td>
                                                <a href="{{ route('smart-block.show', $item->id) }}" class="btn btn-info mr-1 mt-1" data-toggle="tooltip" data-original-title="View"><i class="fas fa-eye"></i></a>
                                                <a href="{{ route('smart-block.edit', $item->id) }}" class="btn btn-light mr-1 mt-1" data-toggle="tooltip" data-original-title="Edit"><i class="fas fa-edit"></i></a>
                                            </td>
                                            <td>S-{{ $item->id }}</td>
                                            <td>{{ $item->title }}</td>
                        ...
                        <div class="card-footer">
                            @if(request()->get('_token'))
                            {{ $smart_block->appends(['_token' => request()->get('_token'),'search' => request()->get('search') ])->links() 								}}
                            @else
                            {{ $smart_block->links() }}
                           @endif
                        </div>
