@extends('admin.layout.app')

@section('content')
<!-- Main Content -->
<div class="main-content">
    <section class="section">
        <div class="section-header">
            <div class="section-header-back">
                <a href="{{ route('pass.index') }}" class="btn btn-icon"><i class="fas fa-arrow-left"></i></a>
            </div>
            <h1>{{ $title ?? '-' }}</h1>
            @include('admin.inc.breadcrumb', ['breadcrumbs' => Breadcrumbs::generate('admin_email_template_crud', 'Edit', route('pass.edit', $email_template->id))])
        </div>

        <div class="section-body">
            @include('admin.inc.messages')
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <form action="{{ route('pass.update', $email_template->id) }}" method="post">
                            @csrf
                            @method('PUT')
                            <div class="card-body">
                                <div class="form-group">
                                    <label for="title">Title</label>
                                    <input type="text" name="title" class="form-control" id="" value="{{ $email_template->title }}">
                                    @if ($errors->has('title'))
                                    <span class="text-danger d-block">
                                        <strong>{{ $errors->first('title') }}</strong>
                                    </span>
                                    @endif
                                </div>
                               
                                <div class="form-group">
                                    <label for="content">Content</label>
                                    <textarea name="content" class="form-control my-editor" id="" cols="30"
                                        rows="10">{!! $email_template->content !!}</textarea>
                                    @if ($errors->has('content'))
                                    <span class="text-danger d-block">
                                        <strong>{{ $errors->first('content') }}</strong>
                                    </span>
                                    @endif
                                </div>

                                @if($email_template->type == 1)
                                <div class="form-group">
                                    <label for="title">Variables</label>
                                    <?php echo '{{competition_name}}, {{student_name}}, {{exam_date}}, {{logo}}, {{competition_venue}}, {{student_id}}, {{dob}}, {{seat_no}}, {{reporting_time}}, {{competition_time}}, {{venue_arrangement}}'; ?>
                                </div>
                                @else
                                <div class="form-group">
                                    <?php echo '{{grading_name}}, {{student_name}}, {{exam_date}}, {{logo}}, {{competition_venue}}, {{student_id}}, {{dob}}, {{mental_seat_no}}, {{abacus_seat_no}},  {{reporting_time}}, {{competition_time}}, {{venue_arrangement}}'; ?>
                                </div>
                                @endif
                                
                                
                            </div>
                            <div class="card-footer text-right">
                                <button type="submit" class="btn btn-primary"><i class="fas fa-save"></i> Save</button>
                            </div>
                        </form>
                    </div>
                </div>
            </div>
        </div>
    </section>
</div>
@endsection
