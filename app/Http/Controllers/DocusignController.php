...
class DocusignController extends Controller
{
    ...
    public function callback(Request $request)
    {
        ...
        return redirect('admin/sp-agreement')->with('success', 'Docusign Succesfully Connected');
        // // For user info
        // $ch1 = curl_init();
        // curl_setopt($ch1, CURLOPT_URL, 'https://account.docusign.com/oauth/userinfo');
    ...
    {
        $buyeDetail = SellerParticular::where('id', $id)->first();
        $buyer = BuyerParticular::where('seller_particular_id', $id)->first();
        $buyerEmail = $buyeDetail->buyer_email; //'bkumarsocial30@gmail.com';
        $buyerName = $buyer->buyer_name ?? $buyerEmail;
        $pdfName = $buyer->pdf_name ?? '';
        if(empty($pdfName)){
        ...
        $sellerName = $buyeDetail->seller_name ?? $buyeDetail->seller_email;
        $sellerEmail = $buyeDetail->seller_email; 
        $demo_docs_path = asset('pdf/' . $filename);
        ...
        $document = new \DocuSign\eSign\Model\Document([ # create the DocuSign document object
            ...
            'name' => 'S&P Document', # can be different from actual file name
