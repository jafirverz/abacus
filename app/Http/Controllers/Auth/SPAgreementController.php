...
class SPAgreementController extends Controller
{
  ...
  public function authorise(Request $request)
  {
    ...
    $params = [
      ...
      'redirect_uri'  => 'https://www.diycars.com/seller-sales-purchase/callback',
      'client_id'     => 'PROD-200501881C-AUTOLINK-SELLERSALESPURCH',
      'client_secret' => 'erTLmLXdOWhMoEBknEx1qDLZTJe0tS8q',
      'code'          => $code,
      'state'          => $state,
    ];
    ...
    $authHeaders = $this->generateSHA256withRSAHeader(
      ...
      'PROD-200501881C-AUTOLINK-SELLERSALESPURCH',
      'erTLmLXdOWhMoEBknEx1qDLZTJe0tS8q'
    );
    ...
    // $userDetails['postcode'] = $details['regadd']['postal']['value'];
    // $userDetails['address'] = $details['regadd']['unit']['value'] . ' '. $details['regadd']['street']['value'] . ' ' .$details['regadd']['floor']['value']. ' ' .$details['regadd']['building']['value'];
    ...
    if(sizeof($details['vehicles']) > 0){
      ...
      $userDetails['originalregistrationdate'] = date('d-m-Y', strtotime($details['vehicles'][0]['originalregistrationdate']['value']));
      ...
      $userDetails['iulabelno'] = $details['vehicles'][0]['iulabelno']['value'];
      ...
      $userDetails['coeexpirydate'] = date('d-m-Y', strtotime($details['vehicles'][0]['coeexpirydate']['value']));
      $userDetails['roadtaxexpirydate'] = date('d-m-Y', strtotime($details['vehicles'][0]['roadtaxexpirydate']['value']));
      $userDetails['openmarketvalue'] = $details['vehicles'][0]['openmarketvalue']['value'];
      $userDetails['firstregistrationdate'] = date('d-m-Y', strtotime($details['vehicles'][0]['firstregistrationdate']['value']));
    ...
    if(!Session::get('myinfospagreement')){
      Session::put('myinfospagreement', $userDetails);
    }else{
      Session::forget('myinfospagreement');
      Session::put('myinfospagreement', $userDetails);
    }
    Session::forget('myinfospbusiness');
    // dd($userDetails);
    // return redirect()->back();
    if(!empty(Session::get('carId'))){
      $carId = Session::get('carId');
      return redirect('/forms/form-details/'.$carId);
    }else{
      return redirect('/forms/form-details');
    }
    // return redirect('/forms/form-details');
    // dd($details);
    // dd($response->getBody()->getContents());
  ...
  public function createPersonRequest($userUniFin, $jwtAccessToken)
  {
    ...
    $params = [
      'attributes' => 'uinfin,name,sex,email,mobileno,regadd,vehicles.vehicleno,vehicles.type,vehicles.make,vehicles.model,vehicles.yearofmanufacture,vehicles.originalregistrationdate,vehicles.enginecapacity,vehicles.type,vehicles.chassisno,vehicles.primarycolour,vehicles.scheme,vehicles.enginecapacity,vehicles.engineno,vehicles.iulabelno,vehicles.nooftransfers,vehicles.minimumparfbenefit,vehicles.coeexpirydate,vehicles.roadtaxexpirydate,vehicles.openmarketvalue,vehicles.firstregistrationdate',
      'client_id' => 'PROD-200501881C-AUTOLINK-SELLERSALESPURCH'
    ];
