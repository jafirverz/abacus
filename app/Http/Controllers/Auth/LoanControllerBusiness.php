...
class LoanControllerBusiness extends Controller
{
  ...
  public function authorise(Request $request)
  {
    ...
    $params = [
      ...
      'redirect_uri'  => 'https://www.diycars.com/mib-car-loan/callback',
      'client_id'     => 'PROD-200501881C-AUTOLINK-MIB-CARLOAN',
      'client_secret' => 'wi311XlsrADNMkXQQXAsCMtBqewm3vKB',
      'code'          => $code,
      'state'          => $state,
    ];
    ...
    $authHeaders = $this->generateSHA256withRSAHeader(
      'https://api.myinfo.gov.sg/biz/v2/token',
      $params,
      $method,
      $headers['Content-Type'],
      'PROD-200501881C-AUTOLINK-MIB-CARLOAN',
      'wi311XlsrADNMkXQQXAsCMtBqewm3vKB'
    );
    ...
    // $response = $client->request('POST', 'https://api.myinfo.gov.sg/biz/v2/token', [
    //   'form_params' => $params,
    //   'headers' => $headers,
    // ]);

    $response = $http->post('https://api.myinfo.gov.sg/biz/v2/token', [
      'form_params' => $params,
      'headers' => $headers,
    ]);
    ...
    $sub = $data['sub'];
    $subEx = explode('_',$sub);
    // $subEx1 = explode('_',$subEx[0]);
    $uen = $subEx[0];
    $uinfin = $subEx[1];
    $details = $this->createPersonRequest($uen, $uinfin, $jwtAccessToken);
    $userDetails = array();
    // dd($details);
    $userDetails['uinfin'] = $details['person']['uinfin']['value'];
    $userDetails['name'] = $details['person']['name']['value'];
    $userDetails['sex'] = $details['person']['sex']['desc'];
    $userDetails['nationality'] = $details['person']['nationality']['code'];
    $userDetails['dob'] = $details['person']['dob']['value'];
    $userDetails['email'] = $details['person']['email']['value'];
    $userDetails['mobileno'] = $details['person']['mobileno']['nbr']['value'];
    $userDetails['areacode'] = $details['person']['mobileno']['areacode']['value'];
    $userDetails['prefix'] = $details['person']['mobileno']['prefix']['value'];
    $userDetails['countryCode'] = $userDetails['prefix'] . $userDetails['areacode'];
    $userDetails['marital'] = $details['person']['marital']['desc'];
    $userDetails['cpfhistory'] = $details['person']['cpfcontributions']['history'] ?? '';
    $userDetails['noahistory'] = $details['person']['noahistory']['noas'] ?? '';
    if(!Session::get('myinfoloanbusiness')){
      Session::put('myinfoloanbusiness', $userDetails);
    }else{
      Session::forget('myinfoloanbusiness');
      Session::put('myinfoloanbusiness', $userDetails);
    }
    Session::forget('myinfoloan');
    // dd($userDetails);
  ...
  public function createPersonRequest($uen, $uinfin, $jwtAccessToken)
  {
    // $uinfin = 'S6005052Z';
    // $url = 'https://test.api.myinfo.gov.sg/biz/v2/entity-person/{uen}/{uinfin}/';
    $url = 'https://test.api.myinfo.gov.sg/biz/v2/entity-person/'.$uen.'/'.$uinfin.'/';
    // $url            = config('myinfo.api.personal') . '/' . $userUniFin . '/';
    $params = [
      'attributes' => 'uinfin,name,sex,nationality,dob,email,mobileno,regadd,marital,cpfcontributions,noahistory',
      'client_id' => 'PROD-200501881C-AUTOLINK-MIB-CARLOAN'
    ];
